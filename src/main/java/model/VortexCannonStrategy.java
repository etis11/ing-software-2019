...
public class VortexCannonStrategy extends AbstractTargetStrategy {
    ...
    public VortexCannonStrategy(int distance, Match match) {
        this.match = match;
...
//tra tutti i player,filtro quelli che hanno distanza di <=1 da visibletiles e l'altra condizione e' che non sia mestesso
    ...
    public List<Player> hittableTargetsBlackHole(Player shooter) {
        return match.getPlayers().stream().filter(player -> gameMap.allVisibleTiles(shooter).stream().filter(tile -> !tile.equals(shooter.getTile())).anyMatch(visible -> (visible.distance(player, match.getMap()) <= 1)) && !player.equals(shooter) && !this.attackedPlayers.contains(player)).collect(Collectors.toList());
    }
